...
TEST_CASE("engine state save and load roundtrip", "[engine_state]") {
    std::string dbPath = spath("eng_state_rt");
    {
        ...
        for(int i=0;i<2;++i){Track t; t.path="state"+std::to_string(i)+".wav"; t.duration=1.0; for(int b=0;b<32;++b) t.fingerprint[b]=(uint8_t)(0xA0+i*16+b); auto r=db->insertTrack(t); REQUIRE(r.has_value()); REQUIRE(db->queueEnqueue(1,*r,-1).has_value());}
    ...
    sqlite3_stmt* st=nullptr; REQUIRE(sqlite3_prepare_v2(h,"SELECT shuffle_enabled,repeat_mode,cursor_pos,volume,shuffle_perm FROM engine_state WHERE id=1",-1,&st,nullptr)==SQLITE_OK);
    ...
    REQUIRE(vol > 0.41); REQUIRE(vol < 0.43);
    ...
    sqlite3_stmt* st2=nullptr; REQUIRE(sqlite3_prepare_v2(h2,"SELECT shuffle_enabled,repeat_mode,volume FROM engine_state WHERE id=1",-1,&st2,nullptr)==SQLITE_OK);
...
TEST_CASE("engine state shuffle toggle clears perm", "[engine_state]") {
    ...
    // verify blob exists
    sqlite3* h=nullptr; REQUIRE(sqlite3_open_v2(dbPath.c_str(),&h,SQLITE_OPEN_READONLY,nullptr)==SQLITE_OK);
    sqlite3_stmt* st=nullptr; REQUIRE(sqlite3_prepare_v2(h,"SELECT shuffle_perm FROM engine_state WHERE id=1",-1,&st,nullptr)==SQLITE_OK);
    REQUIRE(sqlite3_step(st)==SQLITE_ROW);
    REQUIRE(sqlite3_column_bytes(st,0)==2*sizeof(int64_t));
    sqlite3_finalize(st); sqlite3_close(h);
    REQUIRE(eng->setShuffle(false).has_value());
    sqlite3* h2=nullptr; REQUIRE(sqlite3_open_v2(dbPath.c_str(),&h2,SQLITE_OPEN_READONLY,nullptr)==SQLITE_OK);
    REQUIRE(sqlite3_prepare_v2(h2,"SELECT shuffle_perm,shuffle_enabled FROM engine_state WHERE id=1",-1,&st,nullptr)==SQLITE_OK);
    REQUIRE(sqlite3_step(st)==SQLITE_ROW);
    REQUIRE(sqlite3_column_bytes(st,0)==0); // null blob
    REQUIRE(sqlite3_column_int(st,1)==0);
    sqlite3_finalize(st); sqlite3_close(h2);
    eng.reset();
    { std::error_code ec; std::filesystem::remove(dbPath,ec); std::filesystem::remove(dbPath+"-wal",ec); std::filesystem::remove(dbPath+"-shm",ec); }
}
