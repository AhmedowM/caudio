...
TEST_CASE("search FTS finds title and LIKE fallback COLLATE NOCASE", "[db_search]") {
    ...
    for(auto [title,artist]: std::vector<std::pair<std::string,std::string>>{{"Abbey Road","Beatles"},{"Red House","Hendrix"},{"Hello World","Artist"}}){
        Track t; t.path="/tmp/"+title+".mp3"; t.title=title; t.artist=artist; t.album="Album"; for(int b=0;b<32;++b) t.fingerprint[b]=(uint8_t)(title.size()+b);
        auto r=db->insertTrack(t); REQUIRE(r.has_value());
    }
    auto res=search(*db,"Abbey Road",10);
    REQUIRE(res.has_value());
    REQUIRE(res->size()>=1);
    bool found=false; for(auto &t:*res) if(t.title=="Abbey Road") found=true;
    REQUIRE(found);
    // case-insensitive LIKE fallback
    auto res2=search(*db,"hello",10);
    REQUIRE(res2.has_value());
    REQUIRE(res2->size()>=1);
    bool found2=false; for(auto &t:*res2) if(t.title=="Hello World") found2=true;
    REQUIRE(found2);
...
TEST_CASE("searchLike 5 cols COLLATE NOCASE", "[db_search]") {
    auto dbRes=Database::open(":memory:"); REQUIRE(dbRes.has_value());
    auto db=std::move(dbRes.value());
    Track t; t.path="/tmp/x.mp3"; t.title="Foo"; t.artist="BarArtist"; t.album="BazAlbum"; t.genre="Rock"; for(int b=0;b<32;++b) t.fingerprint[b]=(uint8_t)(0xE0+b);
...
TEST_CASE("search FTS escape LIKE wildcards", "[db_search]") {
    auto dbRes=Database::open(":memory:"); REQUIRE(dbRes.has_value());
    auto db=std::move(dbRes.value());
    Track t; t.path="/tmp/y.mp3"; t.title="100% Pure"; for(int b=0;b<32;++b) t.fingerprint[b]=(uint8_t)(0xF0+b);
    REQUIRE(db->insertTrack(t).has_value());
    auto r=search(*db,"100%",10);
    REQUIRE(r.has_value());
    // should find via LIKE fallback with escaped %
    bool found=false; for(auto &tt:*r) if(tt.title=="100% Pure") found=true;
