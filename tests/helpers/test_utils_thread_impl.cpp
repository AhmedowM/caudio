...
bool thread_parallel_10() {
  ...
  for (int i = 0; i < 10; ++i) if (vals[i] != 1) return false;
  return true;
}

bool thread_setname_current() {
  auto r = setThreadName("ca-test");
  if (!r.has_value() && r.error().code != Result::Unsupported && r.error().code != Result::InvalidArg) return false;
  auto r2 = setThreadName("");
  if (!r2.has_value() && r2.error().code != Result::Unsupported && r2.error().code != Result::InvalidArg) return false;
  return true;
}

bool thread_setname_jthread() {
  std::jthread jt([](std::stop_token st) { std::this_thread::sleep_for(std::chrono::milliseconds(50)); while (!st.stop_requested()) std::this_thread::sleep_for(std::chrono::milliseconds(1)); });
  auto r = setThreadName(jt, "worker-1");
  if (!r.has_value() && r.error().code != Result::Unsupported && r.error().code != Result::State) return false;
  jt.request_stop();
  jt.join();
  std::jthread empty;
  auto r2 = setThreadName(empty, "nope");
  if (r2.has_value()) return false;
  if (r2.error().code != Result::State) return false;
  return true;
}
