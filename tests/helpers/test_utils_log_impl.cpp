...
bool log_injected() {
    ...
    Logger logger(
        [&](Level lvl, std::string_view msg) { out.emplace_back(lvl, std::string(msg)); });
    logger.log(Level::Info, "hello {}", 42);
    if (out.size() != 1)
        return false;
    if (out[0].second.compare("hello 42") != 0)
        return false;
    if (out[0].first != Level::Info)
        return false;
    return true;
}
...
bool log_level_filter() {
    std::vector<std::pair<Level, std::string>> out;
    Logger logger([&](Level lvl, std::string_view msg) { out.emplace_back(lvl, std::string(msg)); },
                  Level::Warn);
...
bool log_convenience() {
    ...
    if (out.size() != 4)
        return false;
    if (out[0].first != Level::Debug)
        return false;
    if (out[0].second.compare("dbg 1") != 0)
        return false;
    if (out[1].first != Level::Info)
        return false;
    if (out[2].first != Level::Warn)
        return false;
    if (out[3].first != Level::Error)
        return false;
    return true;
}
...
bool log_toString_level() {
    if (toString(Level::Debug).compare("Debug") != 0)
        return false;
    if (toString(Level::Info).compare("Info") != 0)
        return false;
    if (toString(Level::Warn).compare("Warn") != 0)
        return false;
    if (toString(Level::Error).compare("Error") != 0)
        return false;
    return true;
}
