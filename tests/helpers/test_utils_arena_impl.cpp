...
bool arena_basic() {
  Arena ar{4096};
  if (ar.capacity() == 0) return false;
  void* p1 = ar.allocate(100, 8);
  if (!p1) return false;
  void* p2 = ar.allocate(200, 16);
  if (!p2) return false;
  if ((reinterpret_cast<std::uintptr_t>(p2) & 15) != 0) return false;
  return true;
}
...
bool arena_exhaustion() {
  ...
  if (p4 != nullptr) return false;
  ar.reset();
  void* p5 = ar.allocate(1024, 1);
  if (p5 != p1) return false;
  return true;
}
...
bool arena_alignment() {
  ...
  for (auto al : aligns) {
    ...
    for (int i = 0; i < 10; ++i) {
      ...
      if (!p) return false;
      if ((reinterpret_cast<std::uintptr_t>(p) & (al - 1)) != 0) return false;
    }
  }
  return true;
}
...
bool arena_64b_align() {
  ...
  if (!p) return false;
  if ((reinterpret_cast<std::uintptr_t>(p) & 63) != 0) return false;
  return true;
}
...
bool arena_interleaved() {
  ...
  if ((reinterpret_cast<std::uintptr_t>(q2) & 15) != 0) return false;
  void* q3 = ar.allocate(13, 32);
  if (!q3) return false;
  if ((reinterpret_cast<std::uintptr_t>(q3) & 31) != 0) return false;
...
bool arena_default_64k() {
  Arena ar;
  if (ar.capacity() != 64 * 1024) return false;
  if (ar.remaining() != 64 * 1024) return false;
  void* p = ar.allocate(64 * 1024, 1);
  if (!p) return false;
  if (ar.allocate(1, 1) != nullptr) return false;
  return true;
}
