...
bool queue_busy_on_full() {
    MpscQueue<int> q{2};
    if (!q.push(1).has_value())
        return false;
    if (!q.push(2).has_value())
        return false;
    auto r = q.push(3);
    if (r.has_value())
        return false;
...
bool queue_push_pop_fifo() {
    MpscQueue<int> q{4};
    if (!q.push(10).has_value())
        return false;
    if (!q.push(20).has_value())
        return false;
    auto v1 = q.pop();
    if (!v1.has_value() || *v1 != 10)
        return false;
    auto v2 = q.pop();
    if (!v2.has_value() || *v2 != 20)
        return false;
    auto v3 = q.pop();
    if (v3.has_value())
        return false;
...
bool queue_wrap() {
    ...
    for (int iter = 0; iter < 1000; ++iter) {
        if (!q2.push(iter).has_value())
            return false;
        auto v = q2.pop();
        if (!v.has_value() || *v != iter)
            return false;
    }
    MpscQueue<int> q1{1};
    for (int i = 0; i < 100; ++i) {
        if (!q1.push(i).has_value())
            return false;
        if (q1.push(i).has_value())
            return false;
        auto v = q1.pop();
