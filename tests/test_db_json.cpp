...
TEST_CASE("trackToJson and trackFromJson roundtrip ordered", "[db_json]") {
    ...
    for(int b=0;b<32;++b) t.fingerprint[b]=(uint8_t)b;
...
TEST_CASE("exportJson and importJson roundtrip", "[db_json]") {
    ...
    std::ifstream f(outPath); std::string content((std::istreambuf_iterator<char>(f)),std::istreambuf_iterator<char>());
    REQUIRE(content.find("JsonTrack")!=std::string::npos);
    // import into new db
    auto db2Res=Database::open(":memory:"); REQUIRE(db2Res.has_value());
    auto db2=std::move(db2Res.value());
    auto ir=importJson(*db2,outPath);
    REQUIRE(ir.has_value());
    auto tracks=db2->listTracks(nullptr); REQUIRE(tracks.has_value());
...
TEST_CASE("importJson handles duplicate fingerprint upsert", "[db_json]") {
    ...
    auto tracks=db->listTracks(nullptr); REQUIRE(tracks.has_value());
    REQUIRE(tracks->size()==1);
    { std::error_code ec; std::filesystem::remove(outPath,ec); }
}
...
TEST_CASE("trackFromJson invalid fingerprint falls back", "[db_json]") {
    ...
    bool allZero=true; for(auto b:r->fingerprint) if(b!=0) allZero=false;
    REQUIRE(!allZero);
}
