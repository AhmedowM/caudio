...
static std::string fixturePath(const std::string& name){
    ...
    if(std::filesystem::exists(td)) return td;
#endif
...
TEST_CASE("player seek and volume", "[player_integration]") {
    ...
    AudioOutput::Config cfg; cfg.sampleRate=(*dec)->sampleRate(); cfg.channels=(*dec)->channels(); cfg.ring=&ring;
...
TEST_CASE("player position time-based via Engine steady_clock", "[player_integration]") {
    std::string dbPath = (std::filesystem::temp_directory_path() / ("pl_pos_" + std::to_string((long long)std::chrono::steady_clock::now().time_since_epoch().count()) + ".db")).string();
    {
        ...
        Track t; t.path=fixturePath("sample.wav"); t.duration=2.0; for(int b=0;b<32;++b) t.fingerprint[b]=(uint8_t)(0x55+b);
    ...
    std::error_code ec; std::filesystem::remove(dbPath, ec);
