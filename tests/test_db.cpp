...
TEST_CASE("SQLite create table and insert", "[db_sqlite]") {
  ...
  REQUIRE(rc == SQLITE_OK);
  if (db) {
    const char* sql = "CREATE TABLE tracks (id INTEGER PRIMARY KEY, name TEXT, path TEXT, fingerprint BLOB, library_id INTEGER)";
...
TEST_CASE("SQLite list rows", "[db_sqlite]") {
  ...
  REQUIRE(rc == SQLITE_OK);
  if (db) {
    sqlite3_exec(db, "CREATE TABLE tracks (id INTEGER PRIMARY KEY, name TEXT)", nullptr, nullptr, nullptr);
