...
inline bool shouldMarkPlayedEx(double duration, double pos, bool marked, int pctThr, int secsThr) noexcept {
  if (marked) return false;
  double pct = pctThr > 0 ? (double)pctThr / 100.0 : 0.6;
  double secs = secsThr > 0 ? (double)secsThr : 90.0;
  if (duration > 0.0 && pos / duration >= pct) return true;
  if (pos >= secs) return true;
  return false;
}
...
public:
  ...
  static ExpectedEngine open(std::string_view path, const EngineConfig& cfg = {}) {
    ...
    e->ownsDb_ = true;
    if (auto err = e->init()) return std::unexpected(*err);
    // load state after init so monitor already running but that's ok
    (void)e->loadState();
    // apply volume to output if any
    if (e->output_) e->output_->setVolume(e->state_.volume);
    return e;
  }

  ExpectedVoid attachDatabase(std::shared_ptr<caudio::db::Database> db) {
    if (!db || !db->handle()) return std::unexpected(caudio::utils::makeError(caudio::utils::Result::InvalidArg, "null db"));
    ...
    // also need to keep a non-owning pointer for legacy loadState that expects unique_ptr? we use dbShared_
    if (auto ec = loadStateFromShared()) return std::unexpected(*ec);
    return {};
  }
  ...
  ExpectedVoid attachDb(std::unique_ptr<caudio::db::Database> db) {
    ...
    db_ = std::move(db);
    ownsDb_ = false;
    if (auto ec = loadState()) return std::unexpected(*ec);
    return {};
  }
  ...
  ExpectedVoid play(int64_t queueId = 1) {
    if (!hasDb()) return std::unexpected(caudio::utils::makeError(caudio::utils::Result::State, "no db"));
    if (queueId==0) queueId=1;
    if (!tryLockQueue()) return std::unexpected(caudio::utils::makeError(caudio::utils::Result::Busy, "queue busy"));
    queue_.queueId = queueId;
    caudio::db::Track t;
    auto r = queueNextLocked(t);
    if (!r) { unlockQueue(); return std::unexpected(r.error()); }
  ...
  ExpectedVoid pause() {
    ...
    if (s != PlaybackState::Playing) return std::unexpected(caudio::utils::makeError(caudio::utils::Result::State, "not playing"));
  ...
  ExpectedVoid resume() {
    auto s = playbackState_.load(std::memory_order_acquire);
    if (s != PlaybackState::Paused) return std::unexpected(caudio::utils::makeError(caudio::utils::Result::State, "not paused"));
  ...
  ExpectedVoid seek(double seconds) {
    if (!hasCurrent_.load(std::memory_order_acquire)) return std::unexpected(caudio::utils::makeError(caudio::utils::Result::State, "no track"));
    if (!std::isfinite(seconds) || seconds < 0) return std::unexpected(caudio::utils::makeError(caudio::utils::Result::InvalidArg, "bad seconds"));
    if (decoder_) {
  ...
  ExpectedVoid setVolume(float g) {
    ...
    if (output_) output_->setVolume(g);
    if (hasDb()) {
      state_.cursorPos = (int64_t)queue_.cursor;
      if (db_) (void)saveState(); else (void)saveStateShared();
    }
    return {};
  }

  float volume() const noexcept { return volume_.load(std::memory_order_relaxed); }
  PlaybackState state() const noexcept { return playbackState_.load(std::memory_order_acquire); }
  double duration() const noexcept { return duration_; }
  double position() const noexcept { return currentPositionLocked(); }
  int64_t currentTrackId() const noexcept {
    if (hasCurrent_.load(std::memory_order_acquire)) return currentTrack_.id;
    return state_.currentTrackId;
  }

  std::expected<caudio::db::DbStats, caudio::utils::Error> getStats() {
    if (!hasDb()) return std::unexpected(caudio::utils::makeError(caudio::utils::Result::State, "no db"));
    if (db_) return db_->getStats();
    return dbShared_->getStats();
  }

  std::string lastError() const {
    if (!lastErr_.empty()) return lastErr_;
    return "";
  }

  ExpectedVoid setShuffle(bool on) {
    if (!hasDb()) return std::unexpected(caudio::utils::makeError(caudio::utils::Result::State, "no db"));
    if (!tryLockQueue()) return std::unexpected(caudio::utils::makeError(caudio::utils::Result::Busy, "busy"));
    auto r = setShuffleLocked(on);
    unlockQueue();
    if (!r) return std::unexpected(r.error());
    // push queue changed event
    EngineEvent ev; ev.type = EngineEventType::QueueChanged; ev.queueId = queue_.queueId;
    pushEvent(ev);
    return {};
  }
  ...
  ExpectedVoid setRepeat(RepeatMode m) {
    if (m != RepeatMode::Off && m != RepeatMode::Queue && m != RepeatMode::One)
      return std::unexpected(caudio::utils::makeError(caudio::utils::Result::InvalidArg, "bad repeat"));
    if (!tryLockQueue()) return std::unexpected(caudio::utils::makeError(caudio::utils::Result::Busy, "busy"));
    queue_.repeat = m;
    state_.repeatMode = m;
    state_.cursorPos = (int64_t)queue_.cursor;
    std::optional<caudio::utils::Error> err;
    if (db_) { if (auto e=saveState()) err=e; }
    else { if (auto e=saveStateShared()) err=e; }
    unlockQueue();
    if (err) return std::unexpected(*err);
    return {};
  }
  ...
  ExpectedVoid next() {
    if (!hasDb()) return std::unexpected(caudio::utils::makeError(caudio::utils::Result::State, "no db"));
    // handle repeat one without shuffle without queue lock? match C: if !shuffle && repeat==One && hasCurrent => seek 0 and play
    if (!queue_.shuffle && queue_.repeat == RepeatMode::One && hasCurrent_.load(std::memory_order_acquire)) {
      // seek to 0
      if (decoder_) (void)decoder_->seek(0.0);
      pausePos_ = 0;
      playStart_ = std::chrono::steady_clock::now();
      playbackState_.store(PlaybackState::Playing, std::memory_order_release);
      ...
      EngineEvent ev; ev.type=EngineEventType::TrackStarted; ev.trackId=currentTrack_.id; ev.queueId=queue_.queueId; ev.duration=duration_;
      pushEvent(ev);
      decodeCv_.notify_all();
      return {};
    }
  ...
  std::expected<EngineEvent, caudio::utils::Error> pollEvent() {
    ...
    if (!r) return std::unexpected(caudio::utils::makeError(caudio::utils::Result::NotFound, "no event"));
    return r.value();
  }

  ExpectedVoid drainEvents(EngineEvent* buf, size_t cap, size_t* n) {
    if (!n) return std::unexpected(caudio::utils::makeError(caudio::utils::Result::InvalidArg, "null n"));
    *n = 0;
    if (!buf && cap != 0) return std::unexpected(caudio::utils::makeError(caudio::utils::Result::InvalidArg, "null buf"));
    size_t got = 0;
    while (got < cap) {
      auto r = eventQueue_.pop();
      if (!r) break;
  ...
  PlaybackState getState() const noexcept { return state(); }
...
private:
  ...
  bool tryLockQueue() noexcept {
    int expected = 0;
    return queueLock_.compare_exchange_strong(expected, 1, std::memory_order_acq_rel, std::memory_order_acquire);
  }
  void unlockQueue() noexcept { queueLock_.store(0, std::memory_order_release); }

  double currentPositionLocked() const noexcept {
    if (!hasCurrent_.load(std::memory_order_acquire)) return 0.0;
    auto st = playbackState_.load(std::memory_order_acquire);
    if (st == PlaybackState::Paused) return pausePos_;
    if (st != PlaybackState::Playing) return pausePos_;
  ...
  sqlite3* dbHandle() const noexcept {
    if (db_) return db_->handle();
    if (dbShared_) return dbShared_->handle();
    return nullptr;
  }
  std::shared_mutex* dbMutex() const noexcept {
    if (db_) return &db_->mutex();
    if (dbShared_) return &dbShared_->mutex();
    return nullptr;
  }
  ...
  std::optional<caudio::utils::Error> loadState() {
    auto* h = dbHandle();
    auto* m = dbMutex();
    if (!h || !m) return caudio::utils::makeError(caudio::utils::Result::InvalidArg, "no db");
    std::unique_lock<std::shared_mutex> lk(*m);
    const char* sql = "SELECT shuffle_enabled, repeat_mode, cursor_pos, current_track_id, volume, shuffle_perm FROM engine_state WHERE id=1";
    sqlite3_stmt* stmt=nullptr;
    int rc = sqlite3_prepare_v2(h, sql, -1, &stmt, nullptr);
    if (rc!=SQLITE_OK) { if(stmt) sqlite3_finalize(stmt); return caudio::utils::makeError(caudio::utils::Result::Internal, "prepare failed"); }
    ...
    if (rc==SQLITE_ROW) {
      ...
      if (state_.volume <0 || state_.volume>1) state_.volume=1.0f;
      ...
      if (state_.cursorPos <0) queue_.cursor=0;
      else {
        queue_.cursor = (size_t)state_.cursorPos;
        if (!queue_.perm.empty() && queue_.cursor >= queue_.perm.size()) queue_.cursor=0;
        else if (queue_.perm.empty() && queue_.cursor!=0) queue_.cursor=0;
      }
      queue_.queueId = 1;
    ...
    sqlite3_finalize(stmt);
    if (rc==SQLITE_DONE) {
      state_.shuffleEnabled=0; state_.repeatMode=RepeatMode::Off; state_.cursorPos=0; state_.currentTrackId=0; state_.volume=1.0f;
      volume_.store(1.0f, std::memory_order_relaxed);
      queue_.perm.clear(); queue_.cursor=0; queue_.shuffle=false; queue_.repeat=RepeatMode::Off; queue_.queueId=1;
      return std::nullopt;
    }
    return caudio::utils::makeError(caudio::utils::Result::Internal, "load failed");
  }

  std::optional<caudio::utils::Error> loadStateFromShared() { return loadState(); }
  ...
  std::optional<caudio::utils::Error> saveState() {
    ...
    if (trc!=SQLITE_OK) return caudio::utils::makeError(caudio::utils::Result::Busy, "begin failed");
    const char* sql = "UPDATE engine_state SET shuffle_enabled=?, repeat_mode=?, shuffle_perm=?, cursor_pos=?, current_track_id=?, volume=?, updated=CURRENT_TIMESTAMP WHERE id=1";
    sqlite3_stmt* stmt=nullptr;
    int rc = sqlite3_prepare_v2(h, sql, -1, &stmt, nullptr);
    if (rc!=SQLITE_OK) { sqlite3_exec(h, "ROLLBACK", nullptr, nullptr, nullptr); return caudio::utils::makeError(caudio::utils::Result::Internal, "prepare failed"); }
    ...
    if (queue_.shuffle && !queue_.perm.empty()) {
      if (queue_.perm.size() > (size_t)(std::numeric_limits<int>::max() / (int)sizeof(int64_t))) {
        sqlite3_finalize(stmt); sqlite3_exec(h, "ROLLBACK", nullptr, nullptr, nullptr); return caudio::utils::makeError(caudio::utils::Result::NoMem, "perm too large");
      }
      sqlite3_bind_blob(stmt,3, queue_.perm.data(), (int)(queue_.perm.size()*sizeof(int64_t)), SQLITE_TRANSIENT);
    ...
    rc = sqlite3_step(stmt);
    sqlite3_finalize(stmt);
    if (rc!=SQLITE_DONE) { sqlite3_exec(h, "ROLLBACK", nullptr, nullptr, nullptr); return caudio::utils::makeError(caudio::utils::Result::Internal, "step failed"); }
    rc = sqlite3_exec(h, "COMMIT", nullptr, nullptr, &err);
    if (err) sqlite3_free(err);
    if (rc!=SQLITE_OK) { sqlite3_exec(h, "ROLLBACK", nullptr, nullptr, nullptr); return caudio::utils::makeError(caudio::utils::Result::Internal, "commit failed"); }
    return std::nullopt;
  }
  std::optional<caudio::utils::Error> saveStateShared() { return saveState(); }
  ...
  size_t queueCountLocked(int64_t qid) {
    ...
    sqlite3_stmt* stmt=nullptr;
    if (sqlite3_prepare_v2(h, sql, -1, &stmt, nullptr)!=SQLITE_OK) return 0;
    sqlite3_bind_int64(stmt,1,qid);
    size_t cnt=0;
    if (sqlite3_step(stmt)==SQLITE_ROW) cnt=(size_t)sqlite3_column_int64(stmt,0);
    sqlite3_finalize(stmt);
    return cnt;
  }
  ...
  std::expected<void, caudio::utils::Error> persistShuffleBlobLocked() {
    auto* h = dbHandle();
    auto* m = dbMutex();
    if (!h || !m) return std::unexpected(caudio::utils::makeError(caudio::utils::Result::InvalidArg, "no db"));
    // Use saveState style but only shuffle fields; we reuse saveState for simplicity via direct sql
    std::unique_lock<std::shared_mutex> lk(*m);
    char* err=nullptr;
    int rc = sqlite3_exec(h, "BEGIN IMMEDIATE", nullptr, nullptr, &err);
    if (err) { sqlite3_free(err); err=nullptr; }
    if (rc!=SQLITE_OK) return std::unexpected(caudio::utils::makeError(caudio::utils::Result::Busy, "begin failed"));
    const char* sql="UPDATE engine_state SET shuffle_perm=?, cursor_pos=?, shuffle_enabled=? WHERE id=1";
    sqlite3_stmt* stmt=nullptr;
    rc = sqlite3_prepare_v2(h, sql, -1, &stmt, nullptr);
    if (rc!=SQLITE_OK) { sqlite3_exec(h, "ROLLBACK", nullptr, nullptr, nullptr); return std::unexpected(caudio::utils::makeError(caudio::utils::Result::Internal, "prepare failed")); }
    if (queue_.shuffle && !queue_.perm.empty()) {
      if (queue_.perm.size() > (size_t)(std::numeric_limits<int>::max()/(int)sizeof(int64_t))) {
        sqlite3_finalize(stmt); sqlite3_exec(h, "ROLLBACK", nullptr, nullptr, nullptr); return std::unexpected(caudio::utils::makeError(caudio::utils::Result::NoMem, "perm large"));
      }
      sqlite3_bind_blob(stmt,1, queue_.perm.data(), (int)(queue_.perm.size()*sizeof(int64_t)), SQLITE_TRANSIENT);
    } else sqlite3_bind_null(stmt,1);
    sqlite3_bind_int64(stmt,2, (int64_t)queue_.cursor);
    sqlite3_bind_int(stmt,3, queue_.shuffle?1:0);
    rc = sqlite3_step(stmt);
    sqlite3_finalize(stmt);
    if (rc!=SQLITE_DONE) { sqlite3_exec(h, "ROLLBACK", nullptr, nullptr, nullptr); return std::unexpected(caudio::utils::makeError(caudio::utils::Result::Internal, "step failed")); }
    rc = sqlite3_exec(h, "COMMIT", nullptr, nullptr, &err);
    if (err) sqlite3_free(err);
    if (rc!=SQLITE_OK) { sqlite3_exec(h, "ROLLBACK", nullptr, nullptr, nullptr); return std::unexpected(caudio::utils::makeError(caudio::utils::Result::Internal, "commit failed")); }
    // also update state
  ...
  std::expected<void, caudio::utils::Error> persistCursorLocked() {
    ...
    sqlite3_stmt* stmt=nullptr;
    if (sqlite3_prepare_v2(h, sql, -1, &stmt, nullptr)!=SQLITE_OK) return std::unexpected(caudio::utils::makeError(caudio::utils::Result::Internal, "prepare failed"));
    sqlite3_bind_int64(stmt,1,(int64_t)queue_.cursor);
    int rc = sqlite3_step(stmt);
    sqlite3_finalize(stmt);
    if (rc!=SQLITE_DONE) return std::unexpected(caudio::utils::makeError(caudio::utils::Result::Internal, "step failed"));
    state_.cursorPos = (int64_t)queue_.cursor;
    return {};
  }

  std::expected<caudio::db::Track, caudio::utils::Error> fetchTrackByPosLocked(int64_t qid, int64_t pos) {
    if (qid==0) qid=1;
    auto* h = dbHandle();
    auto* m = dbMutex();
    if (!h || !m) return std::unexpected(caudio::utils::makeError(caudio::utils::Result::InvalidArg, "no db"));
    ...
    {
      std::shared_lock<std::shared_mutex> lk(*m);
      const char* sql="SELECT track_id FROM queue WHERE queue_id=? ORDER BY position LIMIT 1 OFFSET ?";
      sqlite3_stmt* stmt=nullptr;
      if (sqlite3_prepare_v2(h, sql, -1, &stmt, nullptr)!=SQLITE_OK) return std::unexpected(caudio::utils::makeError(caudio::utils::Result::Internal, "prepare failed"));
      sqlite3_bind_int64(stmt,1,qid);
      sqlite3_bind_int64(stmt,2,pos);
      int rc = sqlite3_step(stmt);
      if (rc!=SQLITE_ROW) { sqlite3_finalize(stmt); return std::unexpected(caudio::utils::makeError(caudio::utils::Result::NotFound, "not found")); }
  ...
  std::expected<void, caudio::utils::Error> setShuffleLocked(bool on) {
    bool want = on;
    if (queue_.shuffle == want && !queue_.perm.empty()) return {};
    if (want) {
      queue_.perm.clear();
      queue_.cursor = 0;
      ...
      for (size_t i=0;i<cnt;++i) queue_.perm[i]=(int64_t)i;
  ...
  std::expected<void, caudio::utils::Error> queueNextLocked(caudio::db::Track& out) {
    if (queue_.queueId==0) queue_.queueId=1;
    if (queue_.shuffle) {
      if (queue_.perm.empty()) {
        size_t cnt = queueCountLocked(queue_.queueId);
        if (cnt==0) return std::unexpected(caudio::utils::makeError(caudio::utils::Result::NotFound, "empty queue"));
        auto sr = setShuffleLocked(true);
        if (!sr) return std::unexpected(sr.error());
        if (queue_.perm.empty()) return std::unexpected(caudio::utils::makeError(caudio::utils::Result::Internal, "no perm"));
      }
      if (queue_.cursor >= queue_.perm.size()) {
        if (queue_.repeat == RepeatMode::Queue) {
          ...
          if (queue_.cursor>0 && queue_.cursor <= queue_.perm.size()) idx = queue_.cursor-1;
          ...
          return std::unexpected(caudio::utils::makeError(caudio::utils::Result::NotFound, "end of queue"));
        }
      }
    ...
    if (queue_.repeat == RepeatMode::Queue) {
      ...
      if (db_) dq = db_->queueDequeue(queue_.queueId);
      else dq = dbShared_->queueDequeue(queue_.queueId);
      if (!dq) return std::unexpected(dq.error());
      qi = dq.value();
      if (db_) (void)db_->queueEnqueue(queue_.queueId, qi.trackId, -1);
      else (void)dbShared_->queueEnqueue(queue_.queueId, qi.trackId, -1);
      auto tr = db_ ? db_->getTrack(qi.trackId) : dbShared_->getTrack(qi.trackId);
      if (!tr) return std::unexpected(tr.error());
      out = tr.value();
      return {};
    }
    // normal: dequeue
    {
      auto dq = db_ ? db_->queueDequeue(queue_.queueId) : dbShared_->queueDequeue(queue_.queueId);
      if (!dq) return std::unexpected(dq.error());
      auto tr = db_ ? db_->getTrack(dq.value().trackId) : dbShared_->getTrack(dq.value().trackId);
      if (!tr) return std::unexpected(tr.error());
      out = tr.value();
      return {};
    }
  ...
  std::expected<void, caudio::utils::Error> queuePrevLocked(caudio::db::Track& out) {
    if (!queue_.shuffle) return std::unexpected(caudio::utils::makeError(caudio::utils::Result::NotFound, "prev only shuffle"));
    if (queue_.perm.empty()) return std::unexpected(caudio::utils::makeError(caudio::utils::Result::NotFound, "no perm"));
    if (queue_.cursor <=1) {
      if (queue_.cursor==0) return std::unexpected(caudio::utils::makeError(caudio::utils::Result::NotFound, "at start"));
      queue_.cursor=0;
    } else {
      queue_.cursor -=2;
      if (queue_.cursor >= queue_.perm.size()) queue_.cursor = queue_.perm.size()-1;
    }
    int64_t pos = queue_.perm[queue_.cursor];
    queue_.cursor++;
    (void)persistCursorLocked();
  ...
  std::expected<void, caudio::utils::Error> doPlayTrack(caudio::db::Track& t) {
    ...
    if (!path.empty()) {
      ...
      if (rRes) {
        ...
        if (dRes) {
          ...
          opened = true;
          duration_ = decoder_->totalFrames() ? (double)decoder_->totalFrames() / (double)decoder_->sampleRate() : t.duration;
          if (duration_ <=0) duration_ = t.duration >0 ? t.duration : 1.0;
    ...
    // push event
    EngineEvent ev; ev.type = EngineEventType::TrackStarted; ev.trackId = t.id; ev.queueId = queue_.queueId; ev.duration = duration_; ev.position = 0;
    pushEvent(ev);
    // wake decode
    decodeCv_.notify_all();
    monCv_.notify_all();
  ...
  void preroll() {
    ...
    while (need > 0 && ring_->availableWrite() >= tmp.size() / decoder_->channels()) {
      ...
      if (need > frames) need -= frames; else break;
    }
  }
  ...
  void pushEvent(const EngineEvent& ev) {
    ...
    if (ev.type == EngineEventType::TrackStarted && cbsCopy.onTrackStarted) {
      ...
      double pct = 0;
      if (ev.duration >0) { pct = (ev.position/ev.duration)*100; if(pct<0) pct=0; if(pct>100) pct=100; }
  ...
  void doHistoryMark() {
    if (!hasDb()) return;
    if (!hasCurrent_.load(std::memory_order_acquire)) return;
    if (markedPlayed_.load(std::memory_order_acquire)) return;
    double dur = duration_;
    ...
    if (!detail::shouldMarkPlayedEx(dur, pos, false, pct, secs)) return;
    bool expected = false;
    bool desired = true;
    // CAS exactly-once
    bool exchanged = markedPlayed_.compare_exchange_strong(expected, desired, std::memory_order_acq_rel, std::memory_order_acquire);
    if (!exchanged) return;
    // transaction
    auto* h = dbHandle();
    auto* m = dbMutex();
    if (!h || !m) { markedPlayed_.store(false, std::memory_order_release); return; }
    std::unique_lock<std::shared_mutex> lk(*m);
    char* err=nullptr;
    int rc = sqlite3_exec(h, "BEGIN IMMEDIATE", nullptr, nullptr, &err);
    if (err) { sqlite3_free(err); err=nullptr; }
    if (rc!=SQLITE_OK) { markedPlayed_.store(false, std::memory_order_release); return; }
    // fetch track
    sqlite3_stmt* stmt=nullptr;
    ...
    if (rc!=SQLITE_OK) ok=false;
      ...
      if (sqlite3_step(stmt)==SQLITE_ROW) playCount = sqlite3_column_int64(stmt,1);
      else ok=false;
      sqlite3_finalize(stmt); stmt=nullptr;
    }
    if (!ok) { sqlite3_exec(h, "ROLLBACK", nullptr, nullptr, nullptr); markedPlayed_.store(false, std::memory_order_release); return; }
    // update track
    ...
    if (!ok) { sqlite3_exec(h, "ROLLBACK", nullptr, nullptr, nullptr); markedPlayed_.store(false, std::memory_order_release); return; }
    // history insert
    const char* insSql = "INSERT INTO history (track_id, started_at, completed_at, position_ms, completion_pct, queue_id) VALUES (?,?,?,?,?,?)";
    rc = sqlite3_prepare_v2(h, insSql, -1, &stmt, nullptr);
    if (rc!=SQLITE_OK) ok=false;
    else {
      int64_t posMs = (int64_t)(pos*1000.0);
      double compPct = 0;
      if (dur>0) { compPct = (pos/dur)*100; if(compPct>100) compPct=100; }
      sqlite3_bind_int64(stmt,1, currentTrack_.id);
    ...
    rc = sqlite3_exec(h, "COMMIT", nullptr, nullptr, &err);
    if (err) sqlite3_free(err);
    if (rc!=SQLITE_OK) { sqlite3_exec(h, "ROLLBACK", nullptr, nullptr, nullptr); markedPlayed_.store(false, std::memory_order_release); return; }
  ...
  void engineTick() {
    ...
    if (st == PlaybackState::Playing) {
      ...
      if (now - last >= 500) {
        ...
        EngineEvent ev; ev.type = EngineEventType::Progress;
        ev.trackId = hasCurrent_.load(std::memory_order_acquire) ? currentTrack_.id : state_.currentTrackId;
      ...
      if (duration_ >0) {
        ...
        if (remaining <= gapS && remaining >=0.0) {
          if (hasCurrent_.load(std::memory_order_acquire)) {
            bool expected = false;
            if (gaplessArmed_.compare_exchange_strong(expected, true, std::memory_order_acq_rel, std::memory_order_acquire)) {
              auto nr = next();
              if (!nr) gaplessArmed_.store(false, std::memory_order_release);
            }
          }
        }
      }
  ...
  void monitorLoop(std::stop_token st) {
    ...
    while (!st.stop_requested() && monRun_.load(std::memory_order_acquire)) {
      ...
      engineTick();
      lk.lock();
      monCv_.wait_for(lk, std::chrono::milliseconds(poll), [&st, this]{ return st.stop_requested() || !monRun_.load(std::memory_order_acquire); });
    }
  }
  ...
  void decodeLoop(std::stop_token st) {
    while (!st.stop_requested() && decodeRun_.load(std::memory_order_acquire)) {
      ...
      if (ps != PlaybackState::Playing || !decoder_ || !ring_) {
        ...
        decodeCv_.wait_for(lk, std::chrono::milliseconds(10), [&st, this]{ return st.stop_requested() || playbackState_.load(std::memory_order_acquire)==PlaybackState::Playing; });
        continue;
      }
