...
inline int64_t fileSizeInner(FILE* f) noexcept {
  ...
  if (cur < 0) return -1;
  if (fseek64(f, 0, SEEK_END) != 0) return -1;
  int64_t sz = ftell64(f);
  if (sz < 0) return -1;
  if (fseek64(f, cur, SEEK_SET) != 0) return -1;
  return sz;
}
} // namespace detail
...
class FileReader final : public Reader {
 public:
  ...
  static caudio::utils::Expected<std::unique_ptr<Reader>> open(const std::filesystem::path& path) {
    if (path.empty()) {
      return std::unexpected(caudio::utils::Error{caudio::utils::Result::InvalidArg, "empty path"});
    }
    FILE* f = nullptr;
#if defined(_WIN32)
...
#endif
    if (!f) {
      return std::unexpected(caudio::utils::Error{caudio::utils::Result::NotFound, "cannot open file"});
    }
  ...
  std::size_t read(std::span<std::byte> dst) override {
    if (!file_ || dst.empty()) return 0;
    return std::fread(dst.data(), 1, dst.size(), file_);
  }

  [[nodiscard]] caudio::utils::Expected<void> seek(int64_t offset, int whence) override {
    if (!file_) return std::unexpected(caudio::utils::Error{caudio::utils::Result::InvalidArg, "null file"});
    if (whence != SEEK_SET && whence != SEEK_CUR && whence != SEEK_END) {
      return std::unexpected(caudio::utils::Error{caudio::utils::Result::InvalidArg, "bad whence"});
    }
    int64_t cur = detail::ftell64(file_);
    if (cur < 0) return std::unexpected(caudio::utils::Error{caudio::utils::Result::Internal, "ftell failed"});
    int64_t sz = detail::fileSizeInner(file_);
    if (sz < 0) return std::unexpected(caudio::utils::Error{caudio::utils::Result::Internal, "size failed"});
    int64_t base = 0;
    switch (whence) {
      case SEEK_SET: base = 0; break;
      case SEEK_CUR: base = cur; break;
      case SEEK_END: base = sz; break;
      default: return std::unexpected(caudio::utils::Error{caudio::utils::Result::InvalidArg, "bad whence"});
    }
    int64_t newPos = base + offset;
    if (newPos < 0 || newPos > sz) {
      return std::unexpected(caudio::utils::Error{caudio::utils::Result::InvalidArg, "seek out of range"});
    }
    if (detail::fseek64(file_, newPos, SEEK_SET) != 0) {
      return std::unexpected(caudio::utils::Error{caudio::utils::Result::Internal, "fseek failed"});
    }
    return {};
  }
...
class MemoryReader final : public Reader {
 public:
  ...
  static caudio::utils::Expected<std::unique_ptr<Reader>> open(const std::vector<std::byte>& data) {
    return open(std::span<const std::byte>(data.data(), data.size()));
  }

  static caudio::utils::Expected<std::unique_ptr<Reader>> open(const std::byte* data, std::size_t len) {
    if (len > 0 && data == nullptr) {
      return std::unexpected(caudio::utils::Error{caudio::utils::Result::InvalidArg, "null data"});
    }
  ...
  [[nodiscard]] caudio::utils::Expected<void> seek(int64_t offset, int whence) override {
    ...
    switch (whence) {
      case SEEK_SET: base = 0; break;
      case SEEK_CUR: base = static_cast<int64_t>(pos_); break;
      case SEEK_END: base = static_cast<int64_t>(buf_.size()); break;
      default: return std::unexpected(caudio::utils::Error{caudio::utils::Result::InvalidArg, "bad whence"});
    }
    int64_t newPos = base + offset;
