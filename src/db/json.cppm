...
inline std::string fingerprintToHex(const std::array<uint8_t,32>& fp){
  ...
  for(uint8_t b: fp){ s.push_back(hex[b>>4]); s.push_back(hex[b&0xf]); }
  return s;
}
inline bool hexToFingerprint(std::string_view hex, std::array<uint8_t,32>& out){
  ...
  for(int i=0;i<32;i++){ int hi=hv(hex[i*2]); int lo=hv(hex[i*2+1]); if(hi<0||lo<0) return false; out[i]=(uint8_t)((hi<<4)|lo); }
  return true;
}
inline void genFingerprintFallback(std::string_view path, int64_t id, int64_t size, int64_t mtime, std::array<uint8_t,32>& out){
  ...
  for(int i=0;i<32;i++){ out[i]=(uint8_t)(h >> ((i%8)*8)); h=h*6364136223846793005ULL+1; if(i%8==7) h ^= 0x9e3779b97f4a7c15ULL; }
}
...
export std::expected<Track, caudio::utils::Error> trackFromJson(const ordered_json& j){
  try{
    Track t;
    auto getI64=[&](const char* k, int64_t& out, int64_t def=0){ if(j.contains(k) && !j[k].is_null()){ if(j[k].is_number()) out=j[k].get<int64_t>(); else out=def; } else out=def; };
    auto getInt=[&](const char* k, int& out, int def=0){ int64_t v=def; getI64(k,v,def); out=(int)v; };
    auto getDbl=[&](const char* k, double& out, double def=0){ if(j.contains(k) && !j[k].is_null() && j[k].is_number()) out=j[k].get<double>(); else out=def; };
    auto getStr=[&](const char* k, std::string& out){ if(j.contains(k) && !j[k].is_null() && j[k].is_string()) out=j[k].get<std::string>(); else out.clear(); };
    getI64("id", t.id, 0);
    getI64("size", t.size, 0);
    getI64("mtime", t.mtime, 0);
    getDbl("duration", t.duration, 0);
    { int64_t v; getI64("sample_rate", v, 0); t.sample_rate=(uint32_t)v; }
    { int64_t v; getI64("channels", v, 0); t.channels=(uint32_t)v; }
    { int64_t v; getI64("bitrate", v, 0); t.bitrate=(int32_t)v; }
    ...
    if(t.library_id==0) t.library_id=1;
...
export std::expected<void, caudio::utils::Error> exportJson(Database& db, const std::filesystem::path& outPath){
  ...
  if(!f) return std::unexpected{caudio::utils::makeError(caudio::utils::Result::Io, "cannot open output")};
  f << root.dump(2);
  if(!f) return std::unexpected{caudio::utils::makeError(caudio::utils::Result::Io, "write failed")};
  return {};
}

export std::expected<void, caudio::utils::Error> importJson(Database& db, const std::filesystem::path& inPath){
  std::ifstream f(inPath, std::ios::binary);
  if(!f) return std::unexpected{caudio::utils::makeError(caudio::utils::Result::Io, "cannot open input")};
  std::string content((std::istreambuf_iterator<char>(f)), std::istreambuf_iterator<char>());
  if(content.empty()) return std::unexpected{caudio::utils::makeError(caudio::utils::Result::Corrupt, "empty file")};
  ordered_json root;
  try{
  ...
  if(!root.contains("tracks") || !root["tracks"].is_array()){
    return std::unexpected{caudio::utils::makeError(caudio::utils::Result::Corrupt, "missing tracks array")};
  }
  // transaction for bulk
  {
    std::unique_lock lock(db.mutex());
    sqlite3* h = db.handle();
    if(!h) return std::unexpected{caudio::utils::makeError(caudio::utils::Result::Internal,"no db")};
    char* err=nullptr;
    int rc = sqlite3_exec(h, "BEGIN", nullptr, nullptr, &err);
    if(rc!=SQLITE_OK){ if(err) sqlite3_free(err); return std::unexpected{caudio::utils::makeError(caudio::utils::Result::Internal,"begin failed")}; }
    bool corrupt=false;
    for(auto& j: root["tracks"]){
      auto tr = trackFromJson(j);
      if(!tr){ corrupt=true; break; }
      Track t = *tr;
      // try insert, on AlreadyExists update
      // we need to use db methods without double-locking: we already hold lock, so use raw sqlite directly? Use insert via manual to avoid deadlock.
      // For simplicity unlock and use db.insertTrack (which locks) – but we hold unique_lock, would deadlock. So release lock for each insert.
      // Instead we will commit to using raw sql inside this transaction.
      // To avoid complexity, unlock here and use db.insertTrack outside transaction? Simpler: close transaction and use db methods with re-lock.
      // We'll implement as: unlock, insert, lock again.
      // For now just do direct sql without using db methods to stay inside transaction.
      const char* sql="INSERT INTO tracks (fingerprint, path, size, mtime, duration, sample_rate, channels, bitrate, title, artist, album, album_artist, genre, year, track_num, disc_num, cover_art_path, rating, play_count, last_played, date_added, last_scanned, dirty, library_id, deleted_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
      sqlite3_stmt* stmt=nullptr;
      rc = sqlite3_prepare_v2(h, sql, -1, &stmt, nullptr);
      if(rc!=SQLITE_OK){ corrupt=true; if(stmt) sqlite3_finalize(stmt); break; }
      ...
      if(t.date_added) sqlite3_bind_int64(stmt,21,t.date_added); else sqlite3_bind_null(stmt,21);
      ...
      if(t.deleted_at) sqlite3_bind_int64(stmt,25,t.deleted_at); else sqlite3_bind_null(stmt,25);
      ...
      if(rc==SQLITE_CONSTRAINT){
        ...
        if(sqlite3_prepare_v2(h, sel, -1, &ss, nullptr)==SQLITE_OK){
          ...
          if(sqlite3_step(ss)==SQLITE_ROW){
            ...
            sqlite3_finalize(ss);
            const char* upd="UPDATE tracks SET path=?, size=?, mtime=?, duration=?, sample_rate=?, channels=?, bitrate=?, title=?, artist=?, album=?, album_artist=?, genre=?, year=?, track_num=?, disc_num=?, cover_art_path=?, rating=?, play_count=?, last_played=?, date_added=?, last_scanned=?, dirty=?, library_id=?, deleted_at=? WHERE id=?";
            ...
            if(sqlite3_prepare_v2(h, upd, -1, &us, nullptr)==SQLITE_OK){
              ...
              sqlite3_bind_text(us,16,t.cover_art_path.c_str(),-1,SQLITE_TRANSIENT);
              ...
              if(t.deleted_at) sqlite3_bind_int64(us,24,t.deleted_at); else sqlite3_bind_null(us,24);
    ...
    if(corrupt){
      ...
      return std::unexpected{caudio::utils::makeError(caudio::utils::Result::Corrupt, "import corrupt")};
    }
    rc = sqlite3_exec(h, "COMMIT", nullptr, nullptr, &err);
    if(rc!=SQLITE_OK){ if(err) sqlite3_free(err); sqlite3_exec(h, "ROLLBACK", nullptr, nullptr, nullptr); return std::unexpected{caudio::utils::makeError(caudio::utils::Result::Internal,"commit failed")}; }
  }
  return {};
}
