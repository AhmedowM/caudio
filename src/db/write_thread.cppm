...
struct WriteOp {
  ...
  WriteOp() = default;
  WriteOp(std::string s, sqlite3_stmt* st, std::function<void(std::expected<void, caudio::utils::Error>)> c)
    : sql(std::move(s)), stmt(st), cb(std::move(c)) {}
  WriteOp(const WriteOp&) = delete;
  WriteOp& operator=(const WriteOp&) = delete;
  WriteOp(WriteOp&& o) noexcept : sql(std::move(o.sql)), stmt(o.stmt), cb(std::move(o.cb)) { o.stmt=nullptr; }
  WriteOp& operator=(WriteOp&& o) noexcept {
    if(this!=&o){ sql=std::move(o.sql); if(stmt) sqlite3_finalize(stmt); stmt=o.stmt; o.stmt=nullptr; cb=std::move(o.cb); }
    return *this;
  }
...
public:
  ...
  void close(){
    ...
    while(true){
      auto v = queue_.pop();
      if(!v) break;
      if(v->stmt) sqlite3_finalize(v->stmt);
    }
  }

  std::expected<void, caudio::utils::Error> push(std::string sql, sqlite3_stmt* stmt,
    std::function<void(std::expected<void, caudio::utils::Error>)> cb){
  ...
  std::expected<void, caudio::utils::Error> write(std::string sql,
    std::unique_ptr<void, void(*)(void*)> /*stmt*/, std::function<void(std::expected<void, caudio::utils::Error>)> cb){
    (void)sql; (void)cb;
    return {};
  }
...
private:
  void worker(std::stop_token st){
    while(!st.stop_requested()){
      ...
      cv_.wait_for(lk, std::chrono::milliseconds{200}, [this,&st]{ return !queue_.empty() || st.stop_requested(); });
      ...
      if(op.stmt){
        ...
        if(rc!=SQLITE_DONE && rc!=SQLITE_ROW && rc!=SQLITE_OK){
          err = caudio::utils::makeError(caudio::utils::Result::Corrupt, sqlite3_errmsg(db_));
          ok = false;
        }
        ...
        if(rc!=SQLITE_OK){
          err = caudio::utils::makeError(caudio::utils::Result::Corrupt, e?e:sqlite3_errmsg(db_));
          ok=false;
        }
        if(e) sqlite3_free(e);
      }
      ...
      if(op.cb){
        if(ok) op.cb(std::expected<void, caudio::utils::Error>{});
        else op.cb(std::unexpected{err});
      }
    }
  }
