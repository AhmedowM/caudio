...
public:
    ...
    caudio::utils::Expected<void> listen(const std::filesystem::path& dbPath,
        ...
        HANDLE h = ::CreateNamedPipeW(w.c_str(), kPipeAccessDuplex,
    ...
    void run(std::stop_token st,
        ...
        acceptThread_ = std::jthread([this, st, dispatch](std::stop_token jt) {
            (void)jt;
            while (!st.stop_requested() && !stopSource_.get_token().stop_requested() && running_.load()) {
#ifdef _WIN32
                if (!pipeHandle_ || pipeHandle_ == kInvalidHandle) {
                ...
                }
                {
                    ...
                    clients_.emplace_back([clientHandle, dispatch](std::stop_token ct) {
                        ...
                        if (!reqExp) {
                            ...
                            if (!resExp) reply.result = std::unexpected{resExp.error()};
                            else reply.result = *resExp;
                        }
                        std::string repJson = caudio::cli::serializeReply(reply);
                        auto framed = caudio::cli::frame(repJson);
                        DWORD w2 = 0;
                        ::WriteFile(clientHandle, framed.data(), static_cast<DWORD>(framed.size()), &w2, nullptr);
                ...
                if (sel == 0) continue;
                if (!FD_ISSET(listenFd_, &rfds)) continue;
                ...
                }
                {
                    ...
                    clients_.emplace_back([cfd, dispatch](std::stop_token ct) {
                        ...
                        auto recvExact = [cfd](std::span<std::byte> out) -> bool {
                            std::size_t got = 0;
                            while (got < out.size()) {
                                ::ssize_t n = ::recv(cfd, reinterpret_cast<char*>(out.data()) + got, out.size() - got, 0);
                                if (n <= 0) {
                        ...
                        auto sendAll = [cfd](std::span<const std::byte> data) -> bool {
                            ...
                            while (sent < data.size()) {
                                ::ssize_t n = ::send(cfd, reinterpret_cast<const char*>(data.data()) + sent, data.size() - sent, 0);
                                if (n < 0) {
                                    if (errno == EINTR) continue;
                                    return false;
                                }
    ...
    void shutdown() {
        ...
        if (!socketPath_.empty()) {
            ...
            if (h != kInvalidHandle) ::CloseHandle(h);
        }
#else
        if (listenFd_ >= 0) {
            ::close(listenFd_);
